...
(define (⇒ e)
  ...
  ;; → : e ρ σ t x v → £
  (define (→ e ρ σ t xˢ vˢ)
    (define ς `(,(cdr e) ,ρ ,σ ,t ,xˢ ,vˢ))
    (define £
      (cond
         ...
         (match e
           ...
           [`((,eᶠ ,eᵃ) . ,ℓᶜ)
            ...
            (apply
             set-union
             (set)
             (for*/list ([¢ᶠ (in-set (→ eᶠ ρ σ tᵉ xˢ vˢ))]
                         [¢ᵃ (in-set (→ eᵃ ρ (second ¢ᶠ) tᵉ xˢ vˢ))]
                         [vᶠ (in-set (first ¢ᶠ))]
                         [vᵃ (in-set (first ¢ᵃ))])
               ...
               (→ eᵇ ρᶠ⁺ˣ σᶠ⁺ᵃ⁺ˣ tᵉ x vᵃ)))]
           [`(,(? symbol? x) . ,ℓʳ) #:when (equal? x xˢ) (set `(,(set vˢ) ,σ))]
  ...
  (define (fixed-point)
    ...
    (set-clear! Σ)
    (define £ (→ e (hash) (hash) empty (void) (void)))
