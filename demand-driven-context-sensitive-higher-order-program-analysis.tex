...
We want our analyzer to be as simple as possible, so we want the analyzed language to include few features, but we want our example programs and tests to be readable, so we want to write them in a language with more features. We solve this impasse by defining two languages, a \emph{Core Language}~(§~\ref{section:core-language}) and a \emph{Surface Language}~(§~\ref{section:surface-language}). We define a compiler from the Surface Language into the Core Language in~§~\ref{section:compiling-the-surface-language-into-the-core-language}.

\subsection{Core Language (Labeled Call-by-Value \(λ\)-Calculus) (\texttt{ᶜ})}
...
\begin{itemize}
  ...
  \item The only operation on functions are calling them, for example, in the program \texttt{((λ (x) x) (λ (y) y))} the identity function \texttt{(λ (x) x)} is being called with the argument \texttt{(λ (y) y)} (which happens to be another identity function).
  \item Variables may be referenced, for example, the highlighted \texttt{\highlight{x}} in \texttt{(λ (x) \highlight{x})}. **TODO: CLOSED**
\end{itemize}

, and the only operations are calling a function and referencing a variable.

Our analyzer must be able to distinguish between program points that look the same, for example, the two references to the variable \texttt{f} in the function call \texttt{(f f)}, so we label each program point with a unique number. 

\subsection{Surface Language (Racket Subset) (\texttt{ˢ})}
