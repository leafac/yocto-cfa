...
\begin{itemize}
  \item The only kind of value is the function, for example, \texttt{(λ (x) x)} is a function that simply returns its argument (the identity function). Functions may only receive one argument (unary functions).
  \item The only kind of operation is the function application, for example, \texttt{((λ (x) x) (λ (y) y))} is an application of the function \texttt{(λ (x) x)} (the identity function) with the argument \texttt{(λ (y) y)} (which happens to be another identity function).
  \item Variables may only be referenced after they are defined, for example, the program \texttt{(λ (x) \highlight{x})} is valid because the variable \texttt{x} is only referenced in \texttt{\highlight{x}} after it is defined in \texttt{(λ (x) \_\_\_)}, but the program \texttt{(λ (x) \highlight{y})} is invalid because the variable \texttt{\highlight{y}} is undefined. We say that \texttt{\highlight{x}} is a \emph{closed variable reference} and that the former program is \emph{closed}, but that \texttt{\highlight{y}} is an \emph{open variable reference} and that the latter program is \emph{open}.
\end{itemize}

Our analyzer must disambiguate between program points that look the same, so we label each program point with a unique integer. For example, in the program \texttt{((λ (x) x) (λ (x) x))}) the two \texttt{(λ (x) x)} look the same, but the labels disambiguate them: \texttt{((λ (x) x⁰)¹ (λ (x) x²)³)⁴}. In the code, we represent these labels by pairing the program points with them, for example, \texttt{((((λ (x) (x . 0)) . 1) ((λ (x) (x . 2)) . 3)) . 4)}.

The grammar for the Core Language is the following:
...
\begin{itemize}
  ...
  \item Follow the rules for identifiers from Racket.
\end{itemize}

The Core Language is something called the labeled call-by-value statically-scoped \(λ\)-calculus.

We use the superscript \(ᶜ\) to disambiguate elements of the Core Language from those of other languages, for example, an expression in the Core Language is \(eᶜ\).
