...
\begin{itemize}
  \item The only kind of value is the function, for example, \texttt{(λ (x) x)} is a function that simply returns its argument (the identity function). A function may receive only one argument (unary function).
...
The grammar for the Core Language is the following (refer to \url{http://matt.might.net/articles/grammars-bnf-ebnf/} to learn how to read a grammar):
