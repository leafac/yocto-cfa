...
% TODO: I believe that our analyzer has perfect call–return alignment, but only regular-approximating context sensitivity. This has a huge impact in the rest of what I wrote, starting with the title (which claims that my analyzer is context-sensitive). Upon further reflection (that is, since I start writing the Introduction and whatnot), I believe that call–return alignment is the best we can do, and it doesn’t necessarily translate into context-sensitivity all the time.
