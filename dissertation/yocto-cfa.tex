...
Note how Step~0 produces the expression \mintinline{js}{z => y => y} (see §~\ref{Substitution in Function Definitions}), which does not appear in the original program, but Step~1 produces only expressions that exist in the original program, for example, \mintinline{js}{z => x}.
...
The runner needs to maintain an environment, so we introduce an auxiliary function called \mintinline{ts}{step()} and change all recursive calls to \mintinline{ts}{run()}:

\begin{minted}{ts}
function run(expression: Expression): Value {
  return step(expression, new MapDeepEqual());
  function step(expression: Expression, environment: Environment): Value {
    // The implementation of run() from Step 0 in which recursive calls
    // to run(___) have been replaced with step(___, environment).
  }
}
\end{minted}

The \mintinline{ts}{environment} starts empty (\mintinline{ts}{new MapDeepEqual()}) and for now it is propagated through the recursive calls to \mintinline{ts}{step()}. We must then consider how to manage the \mintinline{ts}{environment} for each different type of \mintinline{ts}{expression}.
...
When the interpreter encounters a function definition, it creates a closure with the current \mintinline{ts}{environment}:
...
\begin{tabular}{c|c|c}
\textbf{Example Program} & \textbf{Current Output} & \textbf{Expected Output} \\
\mintinline{js}{(x => z => x)(y => y)} & — & $\langle \mintinline{js}{}, [] \rangle$ \\
\end{tabular}
\end{center}
