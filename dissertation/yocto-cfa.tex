...
Our interpreter is defined as a function called \mintinline{ts}{evaluate()}. The only parameter to \mintinline{ts}{evaluate()} is a \mintinline{ts}{string} representing a Yocto-JavaScript program. The output of \mintinline{ts}{evaluate()} is a \mintinline{ts}{Value} (recall from §~\ref{Values in Yocto-JavaScript} that functions are the only kind of value in Yocto-JavaScript). The following are two examples of how to use \mintinline{ts}{evaluate()}:
