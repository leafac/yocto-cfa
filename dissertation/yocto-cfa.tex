...
The interpreter we developed in Step~0 may not terminate for some programs (for example, see §~\ref{A Program That Does Not Terminate}). As we progress toward an analyzer, one of our main goals is to guarantee termination: even if the analyzer takes a long time working on a program, we want to make sure that it would eventually finish. In Step~0, the source of non-termination is that the interpreter is constantly producing new expressions as the result of substitution. These new expressions, in turn, must be evaluated, which may produce yet more new expressions.

To visualize this process, consider the following program which is a variation on the program covered from §~\ref{A Program That Does Not Terminate}:
