...
\begin{center}
\begin{tabular}{rrcl}
\textbf{Line} & (see §~\ref{The Entire Runner}) & & \\
...
\begin{center}
\begin{tabular}{rrcl}
\textbf{Line} & (see §~\ref{The Entire Runner}) & & \mintinline{js}{F = f => (f(f))(f(f))} \\
...
\section{A Function Body Is Evaluated with the Environment in Its Closure}

\begin{center}
\begin{tabular}{ll}
\textbf{Example Program:} & \mintinline{js}{(f => (x => f(x))(a => a))((x => z => x)(y => y))} \\
...
This example program shows the difference between the current environment with which an expression is evaluated and the environment that comes from a closure. The following is a trace of the first call to \mintinline{ts}{step()}, when the relevant closure is created:

\begin{center}
\textbf{Trace 1: Closure Creation}
\begin{tabular}{rrcl}
\textbf{Line} & (see §~\ref{A Function Call}) & & \\
& \mintinline{ts}{environment} & = & $[]$ \\
9 & \mintinline{ts}{expression.callee} & = & \mintinline{js}{f => (x => f(x))(a => a)} \\
5 & \mintinline{ts}{parameter} & = & \mintinline{js}{f} \\
6 & \mintinline{ts}{body} & = & \mintinline{js}{(x => f(x))(a => a)} \\
10 & \mintinline{ts}{expression.arguments[0]} & = & \mintinline{js}{(x => z => x)(y => y)} \\
10 & \mintinline{ts}{argument} & = & $\langle \mintinline{js}{(z => x)}, [\mintinline{js}{x} \mapsto \langle \mintinline{js}{(y => y)}, [] \rangle] \rangle$ \\
\end{tabular}
\end{center}

At the top level this program is a call in which the called function is \mintinline{js}{f => (x => f(x))(a => a)} and the argument is \mintinline{js}{(x => z => x)(y => y)}. As discussed in §~\ref{A Function Call}, the argument evaluates to $\langle \mintinline{js}{(z => x)}, [\mintinline{js}{x} \mapsto \langle \mintinline{js}{(y => y)}, [] \rangle] \rangle$, which becomes the closure associated with \mintinline{js}{f}, and when \mintinline{js}{f} is called the current environment includes $\mintinline{js}{x} \mapsto \langle \mintinline{js}{(a => a)}, [\cdots] \rangle$.
