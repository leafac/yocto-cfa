...
The programs that did not terminate in Step~0 (see §~\ref{Step 0: Programs That Do Not Terminate}) continue to not terminate in Step~1, because the interpreters are equivalent except for the technique used to interpret function calls, but the sources of non-termination are different. In Step~0 substitution may produce infinitely many expressions, including expressions that did not occur in the original program. In Step~1 the interpreter considers only the finitely many expressions that occur in the original program, but may produce infinitely many environments.
