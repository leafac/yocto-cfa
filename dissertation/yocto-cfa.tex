...
Our interpreter is defined as a function called \mintinline{ts}{evaluate()}. The only parameter to \mintinline{ts}{evaluate()} is the \mintinline{ts}{Expression} to evaluate, which typically is the output of the parser (see §~\ref{A Parser and Data Structures to Represent Yocto-JavaScript Programs}). The output of \mintinline{ts}{evaluate()} is a \mintinline{ts}{Value}, of which the only kind is a function (see §~\ref{Values in Yocto-JavaScript}). The following is the start of the definition of \mintinline{ts}{evaluate()} (we throw exceptions as placeholders for code that has not been written yet to avoid errors in the TypeScript type checker):
...
To begin the implementation of \mintinline{ts}{evaluate()} we must determine what type of \mintinline{ts}{Expression} it was given:
...
When the \mintinline{ts}{expression} is an \mintinline{ts}{ArrowFunctionExpression} (for example, \mintinline{js}{x => x}), it is already a \mintinline{ts}{Value}, so all \mintinline{ts}{evaluate()} has to do is return it unchanged:
...
For the case in which the \mintinline{ts}{expression} is a \mintinline{ts}{CallExpression}, start by considering a simple call such as \mintinline{js}{(x => x)(y => y)}. As discussed in §~\ref{Operations in Yocto-JavaScript}, we expect the output of this program to be \mintinline{js}{y => y}, which we achieve with the following:
...
Next, consider a program in which the substitution of the variable \mintinline{js}{x} must occur deeper…
