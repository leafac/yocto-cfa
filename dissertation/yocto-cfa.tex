...
Note how Step~0 produces the expression \mintinline{js}{z => y => y} (see §~\ref{Substitution in Function Definitions}), which does not appear in the original program, but Step~1 produces only expressions that exist in the original program, for example, \mintinline{js}{z => x} and \mintinline{js}{y => y}.

\begin{mdframed}[frametitle = {Technical Terms}]
A mapping from parameters to arguments (for example, $[\mintinline{js}{x}\mapsto\mintinline{js}{y => y}]$) is something called an \emph{environment}. A function definition along with an environment (for example, $\langle \mintinline{js}{z => x}, [\mintinline{js}{x}\mapsto\mintinline{js}{y => y}] \rangle$) is something called a \emph{closure}.
\end{mdframed}
...
Note how in each line the two occurrences of \mintinline{ts}{{ name: "Leandro" }} are objects with the same keys and values, but they are not the same object.
...
The listing above does not compile yet because we are not producing closures. In the following sections we fix this by considering how to manage the \mintinline{ts}{environment} for each different type of \mintinline{ts}{expression}.
...
\begin{tabular}{c|c|c}
\textbf{Example Program} & \textbf{Current Output} & \textbf{Expected Output} \\
\mintinline{js}{(x => x => z => x)(a => a)(y => y)} & — & $\langle \mintinline{js}{z => x}, [\mintinline{js}{x} \mapsto \mintinline{js}{y => y}] \rangle$ \\
\end{tabular}
\end{center}

In the case of name reuse (see §~\ref{Step 0: Name Reuse}) the \mintinline{ts}{environment} already contains the \mintinline{ts}{parameter} and the call to \mintinline{ts}{set()} in line~13 of §~\ref{A Function Call} overwrites it. This entails that we  the most recent (the inner one) definition of a name is the one that is used.
...
\mintinline{js}{(x => y)(y => y)} & — & \texttt{Reference to undefined variable: y} \\
\end{tabular}
\end{center}
