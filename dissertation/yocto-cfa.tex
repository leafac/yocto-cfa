...
\begin{center}
\begin{tabular}{rcll}
$e$ & ::= & $x\mintinline{js}{ => }e$ | $e\mintinline{js}{(}e\mintinline{js}{)}$ | $x$ & Expressions \\
$x$ & ::= & <A Valid JavaScript Identifier> & Variables \\
\end{tabular}
\end{center}
...
If the reference to an undefined variable occurs in the body of a function that is not called, then we do not reach the case addressed in this section and an exception is not thrown. For example, the result of running the program \mintinline{js}{x => y} is \mintinline{js}{x => y}, not an exception. This is consistent with JavaScript’s behavior.
...
\mintinline{js}{(f => (f(f))(f(f)))(f => (f(f))(f(f)))} & \mintinline{text}{DOES NOT TERMINATE} & \mintinline{text}{DOES NOT TERMINATE} \\
\end{tabular}
\end{center}
...
\textbf{Step~0} & \mintinline{js}{(x => z => x)(y => y)} & \mintinline{js}{z => (y => y)} \\
\textbf{Step~1} & \mintinline{js}{(x => z => x)(y => y)} & $\langle \mintinline{js}{(z => x)}, [\mintinline{js}{x} \mapsto \mintinline{js}{(y => y)}] \rangle$ \\
\end{tabular}
\end{center}

Step~0 produces the expression \mintinline{js}{z => (y => y)} (see §~\ref{Substitution in Function Definitions}), which does not appear in the original program, but Step~1 produces only expressions that exist in the original program, for example, \mintinline{js}{z => x} and \mintinline{js}{y => y}.

\begin{mdframed}[frametitle = {Technical Terms}]
A map from parameters to arguments (for example, $[\mintinline{js}{x} \mapsto \mintinline{js}{(y => y)}]$) is something called an \emph{environment}. A function definition along with an environment (for example, $\langle \mintinline{js}{(z => x)}, [\mintinline{js}{x} \mapsto \mintinline{js}{(y => y)}] \rangle$) is something called a \emph{closure}.
\end{mdframed}
...
The occurrences of \mintinline{ts}{{ name: "Leandro" }} are objects with the same keys and values, but they are not the same object.

\subsection{Adding an Environment to the Runner}

The runner needs to maintain an environment, so we introduce an auxiliary function called \mintinline{ts}{step()} that receives an \mintinline{ts}{environment} as an extra parameter:
...
\begin{tabular}{c|c|c}
\textbf{Example Program} & \textbf{Current Output} & \textbf{Expected Output} \\
\mintinline{js}{x => x} & — & $\langle \mintinline{js}{(x => x)}, [] \rangle$ \\
\end{tabular}
\end{center}
...
\begin{tabular}{c|c|c}
\textbf{Example Program} & \textbf{Current Output} & \textbf{Expected Output} \\
\mintinline{js}{(x => z => x)(y => y)} & — & $\langle \mintinline{js}{(z => x)}, [\mintinline{js}{x} \mapsto \mintinline{js}{(y => y)}] \rangle$ \\
\end{tabular}
\end{center}
...
\begin{tabular}{c|c|c}
\textbf{Example Program} & \textbf{Current Output} & \textbf{Expected Output} \\
\mintinline{js}{(x => x => z => x)(a => a)(y => y)} & $\langle \mintinline{js}{(z => x)}, [\mintinline{js}{x} \mapsto \mintinline{js}{(y => y)}] \rangle$ & $\langle \mintinline{js}{(z => x)}, [\mintinline{js}{x} \mapsto \mintinline{js}{(y => y)}] \rangle$ \\
\end{tabular}
\end{center}
...
\begin{tabular}{c|c|c}
\textbf{Example Program} & \textbf{Current Output} & \textbf{Expected Output} \\
\mintinline{js}{(x => x)(y => y)} & — & $\langle \mintinline{js}{(y => y)}, [] \rangle$ \\
\mintinline{js}{(x => y)(y => y)} & — & \mintinline{text}{Reference to undefined variable: y} \\
\mintinline{js}{x => y} & — & $\langle \mintinline{js}{(x => y)}, [] \rangle$ \\
\end{tabular}
\end{center}
