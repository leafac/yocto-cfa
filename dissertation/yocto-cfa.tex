...
Our first step toward an analyzer is an interpreter that simply executes Yocto-JavaScript programs producing the same outputs as a regular JavaScript interpreter. This is a good starting point because it is the ground truth against which we will validate the outputs of the analyzer we end up building.

There are several techniques to develop an interpreter, and in this step we use one of the simplest: when the interpreter encounters a function call, it substitutes the variable references in the body of the called function with the argument. This is similar to how we reason about function application in mathematics; for example, given the function $f(x) = x + 1$, we calculate $f(29)$ by substituting the references to $x$ in $f$ with the argument $29$: $f(29) = 29 + 1$. (In technical terms, we are developing something called a \emph{substitution-based interpreter}.)

Despite the simple intuition on which our interpreter is based, there are several edge cases we must consider. In the sections below we introduce progressively more complex programs and adapt the interpreter accordingly.

\subsection{The Skeleton}

Our interpreter is defined as a function called \mintinline{ts}{evaluate()}. The only parameter to \mintinline{ts}{evaluate()} is an \mintinline{ts}{Expression}, which typically is the output of the parser (see §~\ref{A Parser and Data Structures to Represent Yocto-JavaScript Programs}). The output of \mintinline{ts}{evaluate()} is a \mintinline{ts}{Value}, which in Yocto-JavaScript must be a function (see §~\ref{Values in Yocto-JavaScript}). The following is the start of the definition of \mintinline{ts}{evaluate()} (we throw exceptions as placeholders for code that has not been written yet to prevent the TypeScript compiler from signaling errors in our implementation):
...
To begin the implementation of \mintinline{ts}{evaluate()} we must determine the type of \mintinline{ts}{expression}:
...
In this case, the \mintinline{ts}{expression} is already a \mintinline{ts}{Value}, so all \mintinline{ts}{evaluate()} has to do is return it unchanged:
...
For the case in which the \mintinline{ts}{expression} is a \mintinline{ts}{CallExpression}, start by considering a simple call such as \mintinline{js}{(x => x)(y => y)}. As discussed in §~\ref{Operations in Yocto-JavaScript}, we expect the output of this call to be \mintinline{js}{y => y}, which we may achieve with the following:
