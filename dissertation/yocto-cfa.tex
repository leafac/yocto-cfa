...
\begin{center}
\textbf{Trace 1: First Call to \mintinline{ts}{step()} · Closure Creation}
\begin{tabular}{rrcl}
...
And the following is a trace of the recursive call to \mintinline{ts}{step()} in which the relevant closure is used:

\begin{center}
\textbf{Trace 2: Recursive Call to \mintinline{ts}{step()} · Closure Use}
\begin{tabular}{rrcl}
\textbf{Line} & (see §~\ref{A Function Call}) & & \\
...
% TODO: When \mintinline{js}{f} is called the current environment includes $\mintinline{js}{x} \mapsto \langle \mintinline{js}{(a => a)}, [\cdots] \rangle$.
