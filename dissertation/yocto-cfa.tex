...
\begin{center}
\begin{tabular}{rrcl}
\textbf{Line} & \multicolumn{1}{l}{(see §~\ref{The Entire Runner})} & & where \mintinline{js}{F = f => (f(f))(f(f))} \\
...
The program above is a variation on the shortest non-terminating program, \mintinline{js}{(f => f(f))(f => f(f))}, in which each \mintinline{js}{f => f(f)} was modified to receive an additional unused parameter \mintinline{js}{u}, and each \mintinline{js}{f(f)} was modified to receive an additional dummy argument \mintinline{js}{x => x}.
