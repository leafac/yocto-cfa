...
The Yocto-JavaScript parser is implemented as a function called \mintinline{ts}{parse()}, and the following are two examples of how it is used and what data structures it outputs (the \mintinline{ts}{>} represents the console):
...
We choose to represent Yocto-JavaScript programs with the data structures above because they follow a specification called ESTree~\cite{estree}. ESTree is also used by other tools in the JavaScript ecosystem, in particular, it is produced by a parser for JavaScript called Esprima~\cite{esprima}, which we reuse to implement \mintinline{ts}{parse()}. To explore Esprima and ESTree, refer to the Esprima interactive online demonstration~\cite{esprima-demonstration}. The details of how \mintinline{ts}{parse()} is implemented and how it reuses Esprima are deferred to §~\ref{The Yocto-JavaScript Parser in Detail}.

The following are the definitions of data structures produced by \mintinline{ts}{parse()}:
...
These definitions are written as TypeScript types that are adapted from the ESTree specification~\cite{estree-types} to include only the features supported by Yocto-JavaScript. The definitions correspond to elements of the Yocto-JavaScript grammar (see §~\ref{A Formal Grammar for Yocto-JavaScript}); for example, \mintinline{ts}{Expression} corresponds to $e$.
...
There are several techniques to develop an interpreter, and in this step we use one of the simplest: when the interpreter encounters a function call, it substitutes variable references in called function body with the argument. This is similar to how we reason about function application in mathematics; for example, given the function $f(x) = x + 1$, we calculate $f(29)$ by substituting the references to $x$ in $f$ with the argument $29$: $f(29) = 29 + 1$. (In technical terms, we are developing something called a \emph{substitution-based interpreter}.)

\subsection{The Skeleton}

Our interpreter is defined as a function called \mintinline{ts}{evaluate()}. The only parameter to \mintinline{ts}{evaluate()} is an \mintinline{ts}{Expression}, which typically is the output of the parser (see §~\ref{A Parser and Data Structures to Represent Yocto-JavaScript Programs}). The output of \mintinline{ts}{evaluate()} is a \mintinline{ts}{Value}, which in Yocto-JavaScript is a function (see §~\ref{Values in Yocto-JavaScript}). The following is an example of how to use \mintinline{ts}{evaluate()}:
...
The first thing that \mintinline{ts}{evaluate()} has to do is determine the type of \mintinline{ts}{expression} it is given:
...
In this case the \mintinline{ts}{expression} given to \mintinline{ts}{evaluate()} is a \mintinline{ts}{CallExpression} in which both the \mintinline{ts}{callee} and the \mintinline{ts}{argument} are \mintinline{ts}{ArrowFunctionExpression}s. We…
...
In the listing above we deconstruct the \mintinline{ts}{callee} and the \mintinline{ts}{argument}, but we do not use them in any meaningful way; we simply return the \mintinline{ts}{argument}. This is enough to evaluate the program \mintinline{js}{(x => x)(y => y)} correctly.

Next, consider a program in which the substitution of the variable \mintinline{js}{x} must occur within another function definition, for example, \mintinline{js}{(x => z => x)(y => y)}. In this case, we expect the interpreter to substitute the \mintinline{js}{x} in \mintinline{js}{z => x} with \mintinline{js}{y => y}, resulting in \mintinline{js}{z => y => y}. In general, references to the variable to substitute may occur anywhere in the body of the called function, so we define an auxiliary function called \mintinline{ts}{substitute()} to traverse the body looking for these references and performing the substitution:
...
After this recursive call, \mintinline{ts}{substitute()} reaches the case in which the \mintinline{ts}{expression} is an \mintinline{ts}{Identifier}. When this happens, \mintinline{ts}{substitute()} simply returns the \mintinline{ts}{argument}:
...
This definition of \mintinline{ts}{substitute()} is still incomplete, but it is enough to interpret the program we are considering: \mintinline{js}{(x => z => x)(y => y)}. To use
