...
\begin{mdframed}[frametitle = {Technical Terms}]
This example program is also known as the \emph{$\Omega$-combinator}. It is the shortest non-terminating program in Yocto-JavaScript. The function \mintinline{js}{f => f(f)} that is part of the $\Omega$-combinator ($\Omega = U(U)$) is also known as the \emph{$U$-combinator}.
\end{mdframed}
