...
\textbf{Step~0} & \mintinline{js}{(x => y => x)(a => a)} & \mintinline{js}{y => (a => a)} \\
\textbf{Step~1} & \mintinline{js}{(x => y => x)(a => a)} & $\langle \mintinline{js}{y => x}, [\mintinline{js}{x}\mapsto\mintinline{js}{a => a}] \rangle$ \\
\end{tabular}
\end{center}

Note how Step~0 produces the expression \mintinline{js}{y => (a => a)}, which does not appear in the original program, but Step~1 produces only expressions that exist in the original program, for example, \mintinline{js}{y => x}.

\begin{mdframed}[frametitle = {Technical Terms}]
A mapping from parameters to arguments (for example, $[\mintinline{js}{x}\mapsto\mintinline{js}{a => a}]$) is something called an \emph{environment}. A function definition along with an environment (for example, $\langle \mintinline{js}{y => x}, [\mintinline{js}{x}\mapsto\mintinline{js}{a => a}] \rangle$) is something called a \emph{closure}.
\end{mdframed}
