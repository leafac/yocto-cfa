...
We write Yocto-JavaScript programs (see §~\ref{the-analyzed-language-yocto-javascript}) in TypeScript (see §~\ref{the-analyzer-language-typescript}) as strings, for example, \mintinline{ts}{"x => x"}. Our analyzer would be exceedingly complex if it tried to manipulate these strings directly, so it converts them into more convenient data structures. (This is a process called \emph{parsing}, and the data structures are called the \emph{Abstract Syntax Tree}~(AST) of the program~\cite[§~4]{dragon-book}.)

The following are two examples of Yocto-JavaScript programs and the corresponding data structures that the analyzer uses to represent them:
...
See §~\ref{a-parser-and-data-structures-to-represent-yocto-javascript programs} for an introduction of the Yocto-JavaScript parser. This section describes its implementation in further detail.
...
The sole \mintinline{ts}{program} \mintinline{ts}{Statement} must be an \mintinline{ts}{ExpressionStatement}; for example, the program \mintinline{js}{x => x} is allowed because it is an \mintinline{ts}{ExpressionStatement}, but the program \mintinline{js}{const f = x => x} is disallowed because it is a \mintinline{ts}{VariableDeclaration}.
\end{description}

\item [Line~9:]

The analyzer does not need the whole \mintinline{ts}{Program} data structure, but only the sole \mintinline{ts}{Expression} within it, which this line extract. Also, this line is the entry call for the auxiliary function \mintinline{ts}{checkAndClean()} (see below).
