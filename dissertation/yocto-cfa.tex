...
Our interpreter is defined as a function called \mintinline{ts}{evaluate()} that receives the \mintinline{ts}{Expression} to interpret (typically, this \mintinline{ts}{Expression} is the result of calling the parser; see §~\ref{a-parser-and-data-structures-to-represent-yocto-javascript-programs}) and produces a \mintinline{ts}{Value}:
...
As discussed in §~\ref{Values in Yocto-JavaScript}…
