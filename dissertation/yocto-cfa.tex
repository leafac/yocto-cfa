...
As mentioned in §~\ref{Programs That Do Not Terminate}, the interpreter from Step~0 may not terminate for some programs, and preventing non-termination is one of the main issues we must address when developing an analyzer. The source of non-termination in the interpreter from Step~0 is substitution, which may produce infinitely many new expressions. In this Step, we modify the interpreter so that it does not perform substitution, and as a consequence the only expressions it considers are the finitely many expressions and sub-expressions from the input program. The interpreter we develop in this Step may still not terminate, but that would be because of other sources of non-termination that will be addressed in subsequent Steps.

\subsection{Environments and Closures}

In Step~0, when a function was applied we substituted all references to the parameter in the body with the argument. In the interpreter we develop in this Step, we do not perform the substitution, but keep the body intact along with a mapping from the parameter to the argument, for example:

\begin{center}
\begin{tabular}{rcc}
& \textbf{Program} & \textbf{Evaluates to} \\
\textbf{Step~0} & \mintinline{js}{(x => x)(y => y)} & \mintinline{js}{y => y} \\
\textbf{Step~1} & \mintinline{js}{(x => x)(y => y)} & $\langle \mintinline{js}{x}, [\mintinline{js}{x}\mapsto\mintinline{js}{y => y}] \rangle$ \\
\end{tabular}
\end{center}

\begin{mdframed}[frametitle = {Technical Terms}]
A mapping from parameters to arguments (for example, $[\mintinline{js}{x}\mapsto\mintinline{js}{y => y}]$) is something called an \emph{environment}. A function body along with an environment (for example, $\langle \mintinline{js}{x}, [\mintinline{js}{x}\mapsto\mintinline{js}{y => y}] \rangle$) is something called a \emph{closure}.
\end{mdframed}

It is possible to use the outputs of the interpreter from this Step to recreate an expression produced by the interpreter from Step~0 by substituting the variable references in the function body with the corresponding values found in the environment. We may do this to check that the outputs of the interpreters are equivalent. But the interpreter from this Step does not perform substitution: as interpretation proceeds the interpreter simply adds more mappings to the environment, and it only looks up variable references in the environment when necessary.

\begin{mdframed}[frametitle = {Alternative Argument}]
Another way to reason about an environment-based interpreter is as a substitution-based interpreter in which substitutions are delayed until they are needed.
\end{mdframed}
