...
See §~\ref{a-parser-and-data-structures-to-represent-yocto-javascript programs} for an introduction on the Yocto-JavaScript parser. This section describes its implementation in detail.

Most of the parsing work is done by a library called Esprima~\cite{esprima}, which is a parser for JavaScript. Esprima already produces data structures following the ESTree standard. It is also responsible for detecting syntax errors in the input program; for example, it detects that the program \mintinline{js}{x =>} is missing the function body. The rest of the Yocto-JavaScript parser checks whether the JavaScript features used by the parsed program are part of the subset supported by Yocto-JavaScript; for example, it disallows functions of multiple parameters such as \mintinline{js}{(x, y) => x}. The parser also checks that all variable references refer to variables that are defined; for example, it disallows a program such as \mintinline{js}{x => y}, because \mintinline{js}{y} is undefined. Finally, the parser cleans up the data structures generated by Esprima so that they contain only the information that the analyzer needs; for example, it removes location information relating the data structures back to their corresponding segments in the original string input.
...
The parser is defined as a function called \mintinline{ts}{parse()}, which receives the program to be parsed as a \mintinline{ts}{string} called \mintinline{ts}{input}, and produces an \mintinline{ts}{Expression} (see §~\ref{a-parser-and-data-structures-to-represent-yocto-javascript programs} for the definitions of the data structures generated by the Yocto-JavaScript parser).

\paragraph{Line 2:}

Call the Esprima \mintinline{ts}{parseScript()} function which produces ESTree-compliant data structures for the program.

\paragraph{Lines 3–8:}

Check whether the top-level \mintinline{ts}{program} is using only features supported by Yocto-JavaScript:

\subparagraph{Lines 3–4:}

The \mintinline{ts}{program} must include only one \mintinline{ts}{Statement}; for example, the program \mintinline{js}{x => x; y => y} is disallowed.

\subparagraph{Lines 5–8:}
...
Call the auxiliary function \mintinline{ts}{checkAndCleanup()}, which traverses the whole data structure representing the \mintinline{ts}{program} to check whether the features used by it are supported by Yocto-JavaScript and to remove the data included by Esprima that the analyzer does not need, for example, references to which data structures correspond to which segments of the \mintinline{ts}{input}.
