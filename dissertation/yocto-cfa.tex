...
Having chosen Yocto-JavaScript as the analyzed language (see §~\ref{The Analyzed Language: Yocto-JavaScript}) and TypeScript as the language in which to develop the analyzer itself (see §~\ref{The Analyzer Language: TypeScript}), we are ready to start the series of steps in its development.

\subsection{Architecture}

Our interpreter is defined as a function called \mintinline{ts}{evaluate()}, which receives an Yocto-JavaScript program represented as a string and returns the result of running it. (The \mintinline{ts}{evaluate()} function is named after a native JavaScript function called \mintinline{js}{eval()}, which is similar to \mintinline{ts}{evaluate()} but works on JavaScript programs.)
